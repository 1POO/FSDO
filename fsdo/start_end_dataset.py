...
sys.path.append("/media/psr/ZX1 4TB/TR-DETR-master")
import numpy as np
...
class StartEndDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.use_video:
            ...
            model_inputs["video_feat"], model_inputs["object_feat"] = self._get_video_feat_by_vid(
                meta["vid"])  # (Lv, Dv)
            if model_inputs["object_feat"] is None:
        ...
        if self.s_feat_dir is not None:
            ...
            if ctx_ol is not None:
                ctx_vl = min(ctx_vl, ctx_ol)
                model_inputs["video_feat"] = model_inputs["video_feat"][:ctx_vl, :]
                model_inputs["object_feat"] = model_inputs["object_feat"][:ctx_vl, :]
            else:
                ctx_vl = ctx_vl
    ...
    def _get_video_feat_by_vid(self, vid):
        ...
        if self.dset_name == 'tvsum':
            ...
            v_feat_list = [e[:min_len] for e in v_feat_list]
            v_feat = np.concatenate(v_feat_list, axis=1)
            if len(v_objectFeat_dict) == 0:
                return torch.from_numpy(v_feat),None
...
def start_end_collate(batch):
    ...
    for k in model_inputs_keys:
        if batch[0]["model_inputs"][k] is not None:
            if k == "span_labels":
...
def prepare_batch_inputs(opt, batched_model_inputs, device, non_blocking=False):
    if "object_feat" in batched_model_inputs.keys():
        model_inputs = dict(
