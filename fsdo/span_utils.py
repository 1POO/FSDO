...
def generalized_temporal_iou(spans1, spans2):
    ...
    # 将 tensor 转换为 numpy array 并创建 DataFrame
    df_spans1 = pd.DataFrame(df_spans1.cpu().detach().numpy(), columns=["start", "end"])
    df_spans2 = pd.DataFrame(df_spans2.cpu().detach().numpy(), columns=["start", "end"])

    # # 保存为 CSV 文件
    # df_spans1.to_csv("/home/zfw/APP/lppp/TR-DETR-master/results-tacos/spans1.csv", index=False)
    # df_spans2.to_csv("/home/zfw/APP/lppp/TR-DETR-master/results-tacos/spans2.csv", index=False)
