...
class LSTM(nn.Module):
    def __init__(self):
        super().__init__()
        self.lstm = nn.LSTM(256, 512, 2, bidirectional=True, dropout=0.3, batch_first=True)
        self.fc = nn.Linear(1024, 256)
        self.layer_norm = nn.LayerNorm(256)  # 稳定训练

    def forward(self, x):
        residual = x  # (32, 75, 256)
        lstm_out, _ = self.lstm(x)  # (32, 75, 1024)
        out = self.fc(lstm_out)  # (32, 75, 256)
...
class LSTMModule(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, dropout=0.3):
        ...
        self.fc = nn.Linear(2 * hidden_dim, input_dim)  # 維度對齊殘差
        self.layer_norm = nn.LayerNorm(input_dim)

    def forward(self, x, prev_hidden=None):
        residual = x
        lstm_out, hidden = self.lstm(x, prev_hidden)
        lstm_out = self.dropout(lstm_out)
        out = self.fc(lstm_out)
        return self.layer_norm(out + residual), hidden  # 殘差+LayerNorm
...
class StackedLSTM(nn.Module):
    ...
    def __init__(self, num_layers=3, input_dim=256, hidden_dim=512, dropout=0.3):
        super().__init__()
        self.layers = nn.ModuleList([
            LSTMModule(
                input_dim=input_dim if i == 0 else input_dim,  # 所有層輸入維度相同
...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, query_embed, pos_embed, saliency_proj1, video_length=None,
        ...
        if only_encoder:
            ...
            saliency_scores = (torch.sum(saliency_proj1(memory), dim=-1) / np.sqrt(self.d_model)).transpose(0, 1)


            # mamba
            memory = src.permute(1, 0, 2)  # 32,256,75
            memory = self.mamba(memory)
            memory = memory.permute(1, 0, 2)  # 75 32 256
