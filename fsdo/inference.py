...
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath('/home/dell/lppppp/TR-DETR-master_new/fsdo'))))

from fsdo.start_end_dataset_subtitles import start_end_collate_subtitles, prepare_batch_inputs_subtitles, \
    StartEndDataset_subtitles
from tr_detr.start_end_dataset_subtitles_gcn import start_end_collate_subtitles_gcn, prepare_batch_inputs_subtitles_gcn, \
...
from fsdo.config import TestOptions
from fsdo.model import build_model
from fsdo.span_utils import span_cxw_to_xx
from fsdo.start_end_dataset import StartEndDataset, start_end_collate, prepare_batch_inputs
from fsdo.start_end_dataset_audio import \
    StartEndDataset_audio, start_end_collate_audio, prepare_batch_inputs_audio
from fsdo.postprocessing_tr_detr import PostProcessorDETR
...
def compute_hl_results(model, eval_loader, opt, epoch_i=None, criterion=None, tb_writer=None):
    ...
    for batch in tqdm(eval_loader, desc="compute st ed scores"):
        ...
        if opt.s_feat_dir is None:
            model_inputs, targets = prepare_batch_inputs(opt,batch[1], opt.device, non_blocking=opt.pin_memory)
        else:
